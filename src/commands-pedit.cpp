...
bool PCmdColor::execute(Being* being, const std::vector<std::string>& args) {
  ...
  avatar()->send("You have reset %s's {%cgecho color{x!", target->shortname().c_str(), target->gechoColor());
  return true;
}
...
bool PCmdHeadBuilder::execute(Being* being, const std::vector<std::string>& args) {
  ...
  if (target->level() < Being::DEMIGOD) {
    avatar()->send("%s must be promoted at least to level %d before being granted Head Builder status.", target->shortname().c_str(), Being::DEMIGOD);
    return false;
  }
  if (target->adminFlags().toggle(ADMIN_HEADBUILDER)) {
    target->send("You have been granted Head Builder status.");
    avatar()->send("%s has been granted Head Builder status.", target->shortname().c_str());
  } else {
    target->send("You no longer have Head Builder status.");
    avatar()->send("%s no longer has Head Builder status.", target->shortname().c_str());
  }
  return true;
}
