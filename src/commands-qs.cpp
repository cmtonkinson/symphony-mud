...
bool CmdSummary::execute(Being* being, const std::vector<std::string>& args) {
  ...
  avatar()->send("{w||{xrace:   {C%-9s{w ||{xstren: {B%2u{w ||{xarmor: {w%-4d{w   ||{xlevel: {Y%3u{w        ||\n", avatar()->race().string().c_str(), avatar()->strength(), avatar()->armor(), avatar()->level());
  avatar()->send("{w||{xclass:  {C%-9s{w ||{xdexte: {B%2u{w || {xbash:   {w%-4d{w ||{xhealth: {G%4u{x/{g%-4u{w ||\n", avatar()->pClass().string().c_str(), avatar()->dexterity(), avatar()->bash(), avatar()->health(), avatar()->maxHealth());
  avatar()->send("{w||{xgender: {C%-7s{w   ||{xconst: {B%2u{w || {xslash:  {w%-4d{w ||{xmana: {C%4u{x/{c%-4u{w   ||\n", avatar()->gender().string().c_str(), avatar()->constitution(), avatar()->slash(), avatar()->mana(), avatar()->maxMana());
  avatar()->send("{w||{xage:    {C%-3u{w       ||{xintel: {B%2u{w || {xpierce: {w%-4d{w ||{xstamina: {M%3u{w      ||\n", avatar()->age(), avatar()->intelligence(), avatar()->pierce(), avatar()->stamina());
  avatar()->send("{w||{xhand:   {C%-5s{w     ||{xfocus: {B%2u{w || {xexotic: {w%-4d{w ||{xexp: {Y%-7u{w      ||\n", ((avatar()->hand() == Being::WEARLOC_HOLD_R) ? "right" : "left"), avatar()->focus(), avatar()->exotic(), avatar()->exp());
  avatar()->send("{w||{xheight: {C%-9s{w ||{xcreat: {B%2u{w ||              ||{xtnl: {Y%-5u{w        ||\n", "-", avatar()->creativity(), avatar()->tnl());
  avatar()->send("{w||{xweight: {C%-9s{w ||{xchari: {B%2u{w ||              ||{xtrains: {B%-3u{w       ||\n", "-", avatar()->charisma(), avatar()->trains());
  avatar()->send("{w||{xtotem:  {C%-9s{w ||{xluck:  {B%2u{w ||              ||                  ||\n", "-", avatar()->luck());
