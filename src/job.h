...
class Job: public Event {
  public:
    Job( time_t when, EventHandlerBase* what ): _when( when ), _what( what ) { return; }
    template <class EventType>
    Job( time_t when, bool (*function)( EventType* ) ): _when( when ), _what( new EventHandlerFunction<EventType>( function ) ) { return; }
    template <class ObjectType,class EventType>
    Job( time_t when, ObjectType* object, bool (ObjectType::*method)( EventType* ) ): _when( when ), _what( new EventHandlerMethod<ObjectType,EventType>( object, method ) ) { return; }
    virtual ~Job( void );
...
class JobComp {
  public:
    bool operator() ( Job* lhs, Job* rhs ) const { return *lhs < *rhs; }
};
